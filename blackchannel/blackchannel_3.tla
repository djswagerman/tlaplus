    ...
    while TRUE do
        ...
        or 
            SendMessage:
                ...
                Send (messageChannel, sendMessage);
        or
...
startReceiver:
    while TRUE do
        either
            ...
            acknowkledgeChannel := Append (acknowkledgeChannel, receivedMessage);
        or
            skip;
        end either;
    end while;
...
\* BEGIN TRANSLATION (chksum(pcal) = "e9cb78fd" /\ chksum(tla) = "9154db6a")
...
startReceiver(self) == /\ pc[self] = "startReceiver"
                       /\ \/ /\ Len(messageChannel) > 0
                             ...
                             /\ acknowkledgeChannel' = Append (acknowkledgeChannel, receivedMessage'[self])
                          \/ /\ TRUE
                             /\ UNCHANGED <<messageChannel, acknowkledgeChannel, receivedMessage>>
                       /\ pc' = [pc EXCEPT ![self] = "startReceiver"]
                       /\ UNCHANGED << message, sendCount, receiveCount, 
...
\* Last modified Sun Mar 06 13:22:04 CET 2022 by dirk-janswagerman
